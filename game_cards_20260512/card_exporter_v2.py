...
def write_goal_csv(file_path, goals):
    ...
    for goal in goals:
        ...
        # 生成text_bottom，参考格式：{jin}{control}[巴蜀]{district}：8vp\n{player}{control}[巴蜀]{district}：16vp
        text_bottom = f"{{jin}}{{control}}[{goal['simple'].replace('东晋控制[', '').replace(']区域', '')}]{{district}}：{simple_score}vp{{newline}}{{player}}{{control}}[{goal['complete'].replace('玩家控制[', '').replace(']区域', '')}]{{district}}：{complete_score}vp"
        rows.append([
            1,
            goal['name'],
            text_bottom,
