# HexMachina Agent Prompt 原文

> 来源：`agents/agentEvolver_v2/prompts.py`
> 这是 HexMachina 项目中所有 Agent System Prompt 的逐字原文（模板变量 `{FOO_TARGET_FILENAME}` / `{ANALYZER_NAME}` 等 在运行时被替换）
...
每个 Agent 的 System Prompt 前面都会拼接这段文本，后面接角色名。例如 ANALYZER 收到的实际 System Prompt 以
`"You are apart of a multi-agent system... Your specific role is the: ANALYZER"` 开头。
...
### META 附带的 DEFAULT_ANALYZE_MSG（作为 HumanMessage 注入 ANALYZER）
...
Discovery 阶段有独立的 5 个 System Prompt，核心差异是 META 的角色从 "Lead Scientist" 变为 "Lead API Architect"，代理的分工变为围绕构建 `adapters.py` 文件。共性前缀：
